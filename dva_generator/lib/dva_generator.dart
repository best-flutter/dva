...
class DvaAnnotation extends Builder {
  ...
  Iterable<ClassElement> withAnnotation(
      LibraryReader reader, TypeChecker typeChecker) sync* {
    for (AnnotatedElement annotatedElement
        in reader.annotatedWith(typeChecker)) {
  ...
  Iterable<String> handleModel(ClassElement modelClass, DartObject a) sync* {
    ...
    if (superName == 'BaseModel') {
      ...
      for (MethodElement method in modelClass.methods) {
        ...
        } else if (returnType.name.startsWith("Future") ||
  ...
  Iterable<String> generateContent(LibraryReader reader, BuildStep buildStep) {
    ...
    for (ClassElement element in reader.classElements) {
      ...
      for (ElementAnnotation annotation in element.metadata) {
        DartObject a =
            modelType.firstAnnotationOf(element, throwOnUnresolved: true);
        if (a != null) {
  ...
  Future build(BuildStep buildStep) async {
    ...
    if (content.length > 0)
      buildStep.writeAsString(
          buildStep.inputId.changeExtension(".dva.dart"), content.join("\n"));
    /*
